...
【絶対に守るべきルール】
1. 英語テキストは全て「横書き」(left-to-right horizontal) で記載すること。文字を縦に並べたり回転させたりしないこと。
2. 吹き出しのサイズや形を横書きテキストに合わせて調整すること。必要に応じて吹き出しを横長に変形させること。
3. 擬音・効果音は元の位置にダイナミックなレタリングで配置すること。
4. 元の画像の構図、キャラクター、背景は可能な限り維持すること。
5. テキストは読みやすいフォントサイズで、吹き出し内に収まるように配置すること。
6. タイトルは目立つ英語タイポグラフィで表現すること。`;
